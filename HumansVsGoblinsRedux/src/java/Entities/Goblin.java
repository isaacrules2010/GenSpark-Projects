...
public class Goblin extends Creature {
    ...
    public String attack(Creature target) {
        ...
        if(toHit >= target.armorClass){
            ...
            text += ("The Creature hit");
            if((strScore + atk) <= 0)
                text += (", but was too weak to do any damage...\n");
            else text += (" for "+(strScore+atk)+" damage!\n");
            targetHealth = targetHealth - (strScore + atk);
        }
        else{
            text += ("Whiff! The Creature missed!\n");
        }
    ...
    public String pickUp(Item item){
        ...
        return "A goblin picked up an Item!\n";
    }
